 ...
 *        width=20
 *
 * NOTE: Please use canvas coordinate space (https://d  eveloper.mozilla.org/en-US/docs/Web/API/Canvas_API/Tutorial/Drawing_shapes#The_grid),
...
function isCreditCardNumber(ccn) {
  ...
  for (let i = 1; i <= str.length; i += 1) {
    const cardNumber = +str[i - 1];
    if (i % 2 === parity) {
      sum += cardNumber;
    } else {
      sum += cardNumber > 4 ? cardNumber * 2 - 9 : cardNumber * 2;
    }
  }
  return sum % 10 === 0;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (stack.includes(bracketsPair[str[i]])) {
      if (stack.length && stack[stack.length - 1] === bracketsPair[str[i]]) {
        stack.pop();
      } else {
        return false;
      }
